...
export const up = async (db: knex): Promise<any> => {
  await db.schema.createTableIfNotExists('clients', (table: knex.CreateTableBuilder) => {
  ...
  await db.schema.createTableIfNotExists('links', (table: knex.CreateTableBuilder) => {
    table.increments('id').primary();
